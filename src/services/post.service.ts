...
export async function createPost(
  ...
  if (input.platform_credential_id) {
    credQuery = credQuery.eq("id", input.platform_credential_id);
  } else {
    credQuery = credQuery
  ...
  const { data: credRow, error: credError } = await credQuery.limit(1).maybeSingle();
  ...
  const { data: post, error: postError } = await supabase
    ...
    .insert({
      ...
      platform_credential_id: input.platform_credential_id ?? null,
    })
    .select()
    .single();
...
export async function checkAndPublishPost(
  ...
  if (typedPost.platform_credential_id) {
    credQuery = credQuery.eq("id", typedPost.platform_credential_id);
  } else {
    credQuery = credQuery
  ...
  const { data: credRow } = await credQuery.limit(1).maybeSingle();
