    ...
    fields: [
      {
        ...
        type: "text",
        icon: Briefcase,
        helpText: "A descriptive name to identify this account on our website",
      },
      {
        key: "account_id",
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
      ...
      {MANUAL_PLATFORMS.map((platform) => {
        const matching = initialCredentials.filter((c) => c.platform === platform.id);
        return (
          <ManualCredentialsCard
            key={platform.id}
            platform={platform}
            initialCredentials={matching}
          />
        );
      })}
